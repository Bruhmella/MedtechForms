...
use App\Models\misc4;
use App\Models\BasicPatData;
use App\Models\Account;
use Illuminate\Support\Facades\DB;

class Misc4Controller extends Controller
{
        public function create()
    {
        ...
        $latestRecord = misc4::orderBy('OR', 'desc')->first();
     ...
     public function store(Request $request)
    {
     ...
     //       'lotno2' =>$request->lotno2,
       //     'lotno3' =>$request->lotno3,
        ...
        return redirect()->route('misc4.create')->with('success', 'Data saved successfully.');
    }
    ...
    private function generateOrNumber($latestRecord)
    {
        ...
        if ($latestRecord && isset($latestRecord->OR) && preg_match("/^MCIV$datePart(\d{4})$/", $latestRecord->OR, $matches)) {
            $lastNumber = (int) $matches[1] + 1;
        }

        return "MCIV" . $datePart . str_pad($lastNumber, 4, '0', STR_PAD_LEFT);
    }
